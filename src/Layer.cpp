...
Layer nn::Loss(LossFunc loss_f, string name)
{
	Layer info(name);
	info.type = LOSS;
...
Layer nn::Loss(ReduceType loss_f, string name)
{
	Layer info(name);
	info.type = LOSS;
	info.loss = LossInfo(loss_f);
...
Layer nn::Loss(LossFunc f, LossFunc df, bool ignore_activate, string name)
{
	Layer info(name);
	info.type = LOSS;
	info.loss.f = f;
	info.loss.df = df;
	info.loss.ignore_acctive = ignore_activate;
	return info;
}
